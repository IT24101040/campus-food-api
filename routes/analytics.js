...
router.get('/total-spent/:studentId', async (req, res) => {
  try {
    ...
    console.error('Error in total-spent:', err.message);
...
router.get('/top-menu-items', async (req, res) => {
  try {
    ...
    console.error('Error in top-menu-items:', err.message);
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.get('/daily-orders', async (req, res) => {
  try {
    ...
    const formatted = results.map((r) => ({ date: r._id, orderCount: r.orderCount }));
    res.json(formatted);
  } catch (err) {
    console.error('Error in daily-orders:', err.message);
    res.status(500).json({ error: 'Server error' });
  }
});
